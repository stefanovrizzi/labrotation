...
# File extracting recordings to fill out the dataframe
...
def fill_df(n_ch_tot, n_ch, seg, task_names, folder, fs, fsv):
    ...
    for i in range(n_ch_tot): #add channels as columns
    ...
    for task_num, task_name in enumerate(task_names): #load file names per task
    ...
    for file_num, file_name in enumerate(file_list):
        ...
        t_i = int((seg[file_num][0]-1)/fsv *fs) #time segmentation computing frames from VICON to EMG
        t_f = int((seg[file_num][1]-1)/fsv *fs)
        
        array1_temp = np.array(mat["EMG_array"][0][0]['data'][0][0]) #load recordings
